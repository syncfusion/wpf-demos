...
namespace DiagramBuilder.View
{
    ...
    public class DesignVariantRibbonGalleryItem : RibbonGalleryItem
    {
            ...
            DependencyPropertyChangedEventArgs e)
        {
            if ((bool)e.NewValue)
            {
                this.ContentTemplate = Application.Current.Resources["ribbonVariantItemTemplate"] as DataTemplate;
            }
            else
            {
                switch (this.Name)
                {
                    case "VariantGallery0":
                        this.ContentTemplate =
                            Application.Current.Resources["disabledRibbonVariantItemTemplate0"] as DataTemplate;
                        break;
                    case "VariantGallery1":
                    case "VariantGallery2":
                    case "VariantGallery3":
                        this.ContentTemplate =
                            Application.Current.Resources["disabledRibbonVariantItemTemplate1"] as DataTemplate;
                        break;
                }
            }
        }
