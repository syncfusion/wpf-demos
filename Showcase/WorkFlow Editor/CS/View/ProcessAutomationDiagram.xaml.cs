    ...
    public sealed partial class ProcessAutomationDiagram : UserControl
    {
        ...
        private void InitializeDiagram()
        {
           ...
           sfdiagram.SnapSettings.SnapConstraints = SnapConstraints.All;
            ...
            QuickCommandViewModel quickCommand1 = new QuickCommandViewModel()
            {
                ...
                using (FileStream fileStream = File.OpenRead(pathString + "/" + param.ToString() + ".xml"))
                {
                    ...
                    foreach (ProcessAutomationNode node in ((IEnumerable<object>)sfdiagram.Nodes).OfType<ProcessAutomationNode>())
                    {
                        ...
                        if (node.NodeContent != null)
                        {
                            ...
                            {
                                node.ContentTemplate = App.Current.Resources["tasknodetemplate"] as DataTemplate;
                            }
                            else
                            {
                                if (node.NodeType == "Decision")
                                {
                                    node.ContentTemplate = App.Current.Resources["decisionnodetemplate"] as DataTemplate;
                                }
                                else
                                {
                                    if ((node.Content as NodeContent).DispalyText == "Start")
                                    {
                                        node.ContentTemplate = App.Current.Resources["Startnodetemplate"] as DataTemplate;
                                    }
                                    else if ((node.Content as NodeContent).DispalyText == "End")
                                    {
                                        node.ContentTemplate = App.Current.Resources["Endnodetemplate"] as DataTemplate;
                                    }
                                    else
                                    {
                                        node.ContentTemplate = App.Current.Resources["nodetemplate"] as DataTemplate;
                                    }
                                }
                            }
                        }
