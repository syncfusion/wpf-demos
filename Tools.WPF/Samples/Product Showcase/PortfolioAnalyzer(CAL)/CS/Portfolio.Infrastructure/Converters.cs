...
namespace PortfolioAnalyzer.Infrastructure
{
    ...
    public class VolumeConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, System.Globalization.CultureInfo culture)
        {
            ...
            if (x != null)
            {
                double v = System.Convert.ToDouble(x.Content);
    ...
    /// <summary>
    /// Converter for the Candle chart interior color specification
    /// </summary>
    public class ValueConverter : IValueConverter
    {
        public Object Convert(Object value, Type targetType, Object parameter, CultureInfo culture)
        {
            if (value != null)
            {
                ChartColumnSegment cs = value as ChartColumnSegment;
                double open = cs.CorrespondingPoints[0].DataPoint.Values[0];
                double close = cs.CorrespondingPoints[0].DataPoint.Values[1];
                var converter = new System.Windows.Media.BrushConverter();
                if (open < close)
                {
                    var brush = (Brush)converter.ConvertFromString("#B93E3E");
                    return brush;
                }
                else
                {
                    var brush = (Brush)converter.ConvertFromString("#6A9522");
                    return brush;
                }
