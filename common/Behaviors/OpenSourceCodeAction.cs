...
namespace syncfusion.demoscommon.wpf
{
    ...
    {
        /// <summary>
        /// Gets or sets the property value indicating whether it is store package or not
        /// </summary>
        private bool isStorePackage = false;

        protected override void Invoke(object parameter)
        {
            if (this.AssociatedObject == null)
                return;
            if (isStorePackage && AssociatedObject.Name == "opengithub")
            {
                AssociatedObject.Visibility = Visibility.Visible;
            }
            else if (!isStorePackage && AssociatedObject.Name == "openvisualstudio")
            {
