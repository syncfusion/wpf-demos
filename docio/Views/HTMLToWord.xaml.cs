...
namespace syncfusion.dociodemos.wpf
{
    ...
    public partial class HTMLToWord : DemoControl
    {
        ...
        public HTMLToWord()
        {
            ...
            this.textBox1.Text = "HTML to Word.html";
        }
        #endregion
                ...
                if (this.textBox1.Text != String.Empty && fullPath != string.Empty)
                {
                    ...
                    if (File.Exists(fullPath))
                    {
                        ...
                        if (htmlText != string.Empty)
                        {
                            ...
                            else if (section.Body.IsValidXHTML(htmlText, XHTMLValidationType.None, out errorMessage))
                            {
                                valid = true;
                            ...
                            else
                            {
                                ...
                                if (worddoc.IsChecked.Value)
                                {
                                    try
                                    {
                                        ...
                                        document.Save("HTML to Word.doc");
                                        ...
                                        if (MessageBox.Show("Do you want to view the generated Word document?", "Document has been created", MessageBoxButton.YesNo, MessageBoxImage.Information) == MessageBoxResult.Yes)
                                        {
                                            try
                                            {
                                                ...
                                                process.StartInfo = new System.Diagnostics.ProcessStartInfo("HTML to Word.doc") { UseShellExecute = true };
                                                process.Start();
                                            }
                                    ...
                                    catch (Exception ex)
                                    {
                                        if (ex is IOException)
                                            MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\HTML to Word.doc" + ") then try generating the document.", "File is already open",
                                ...
                                else if (worddocx.IsChecked.Value)
                                {
                                    try
                                    {
                                        ...
                                        document.Save("HTML to Word.docx", FormatType.Word2013);
                                        //Message box confirmation to view the created document.
                                        if (MessageBox.Show("Do you want to view the generated Word document?", "Document has been created", MessageBoxButton.YesNo, MessageBoxImage.Information) == MessageBoxResult.Yes)
                                        {
                                            try
                                            {
                                                //Launching the MS Word file using the default Application.[MS Word Or Free WordViewer]
                                                System.Diagnostics.Process process = new System.Diagnostics.Process();
                                                process.StartInfo = new System.Diagnostics.ProcessStartInfo("HTML to Word.docx") { UseShellExecute = true };
                                                process.Start();
                                            }
                                            catch (Win32Exception ex)
                                            {
                                    ...
                                    {
                                        if (ex is IOException)
                                            MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\HTML to Word.docx" + ") then try generating the document.", "File is already open",
                                                 MessageBoxButton.OK, MessageBoxImage.Error);
                                        else
                                            MessageBox.Show("Document could not be generated, Could you please email the error details with along input document to support@syncfusion.com for trouble shooting" + "\r\n" + ex.ToString(), "Error",
                                                    MessageBoxButton.OK, MessageBoxImage.Error);
