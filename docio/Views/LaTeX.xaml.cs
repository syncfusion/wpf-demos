...
namespace syncfusion.dociodemos.wpf
{
    ...
    /// </summary>
    public partial class LaTeX : DemoControl
    {
        # region Constructor
        /// <summary>
        /// Window constructor
                ...
                using (WordDocument document = new WordDocument())
                {
                    ...
                    if (worddocx.IsChecked.Value)
                    {
                        try
                        {
                            ...
                            document.Save("LaTeXToMath.docx", FormatType.Docx);
                            ...
                            if (MessageBox.Show("Do you want to view the generated Word document?", "Document has been created", MessageBoxButton.YesNo, MessageBoxImage.Information) == MessageBoxResult.Yes)
                            {
                                try
                                {
                                    ...
                                    process.StartInfo = new System.Diagnostics.ProcessStartInfo("LaTeXToMath.docx") { UseShellExecute = true };
                                    process.Start();
                                }
                        ...
                        catch (Exception ex)
                        {
                            if (ex is IOException)
                                MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\LaTeXToMath.docx" + ") then try generating the document.", "File is already open",
                    ...
                    else if (pdf.IsChecked.Value)
                    {
                        try
                        {
                            ...
                            if (MessageBox.Show("Do you want to view the generated PDF?", " Document has been created", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                            {
                                ...
                                {
                                    System.Diagnostics.Process process = new System.Diagnostics.Process();
                                    process.StartInfo = new System.Diagnostics.ProcessStartInfo("LaTeXToMath.pdf") { UseShellExecute = true };
                                    process.Start();
                        ...
                        {
                            if (ex is IOException)
                                MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\LaTeXToMath.pdf" + ") then try generating the document.", "File is already open",
                                     MessageBoxButton.OK, MessageBoxImage.Error);
                            else
                                MessageBox.Show("Document could not be generated, Could you please email the error details to support@syncfusion.com for trouble shooting" + "\r\n" + ex.ToString(), "Error",
                                        MessageBoxButton.OK, MessageBoxImage.Error);
