...
namespace syncfusion.dociodemos.wpf
{
    ...
    public partial class WordToHTML : DemoControl
    {
        ...
        public WordToHTML()
        {
            ...
            this.textBox1.Text = "Word to HTML.doc";
        }
        #endregion
                ...
                if (File.Exists(fullPath))
                {
                    try
                    {
                        ...
                        using (WordDocument doc = new WordDocument(fullPath))
                        {
                            ...
                            if (MessageBox.Show("Conversion Successful. Do you want to view the HTML File?", "Status", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                            {
                                ...
                                process.StartInfo = new System.Diagnostics.ProcessStartInfo("Word to HTML.html") { UseShellExecute = true };
                                process.Start();
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        if (ex is IOException)
                            MessageBox.Show("Please close the file (" + Directory.GetCurrentDirectory() + "\\Word to HTML.html" + ") then try generating the document.", "File is already open",
