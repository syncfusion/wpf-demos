    ...
    public class DialogEditingBehavior : Behavior<DialogEditing>
    {
        ...
        private void ShowCardEditDialog(KanbanModel selectedcard)
        {
            ...
            if (editCardDialog.DialogResult == true && editCardDialog.Action == DialogAction.Save)
            {
                ...
                if (selectedcard.Assignee != card.Assignee || selectedcard.Category.ToString() != card.Category.ToString())
                {
                ...
                {
                    selectedcard.Title = editCardDialog.TitleTextBox.Text;
                    selectedcard.ColorKey = (editCardDialog.PriorityComboBox.SelectedItem as ComboBoxItem)?.Content.ToString();
                    selectedcard.Description = editCardDialog.SummaryTextBox.Text;
                }
            }
