...
namespace syncfusion.kanbandemos.wpf
{
    ...
    public class KanbanProductDemos : ProductDemo
    {
        public KanbanProductDemos()
        {
            ...
            this.Demos.Add(new DemoInfo() { SampleName = "Sorting", GroupName = "KANBAN BOARD", Description = "This sample showcases the sorting behavior of the Kanban board using the sort by, custom field, and sort order properties. It supports sorting by item source order, index, or custom field mapping, allowing cards to be arranged and repositioned dynamically or based on predefined values. The sort order property controls whether cards are aligned in ascending or descending order.", DemoViewType = typeof(Sorting) });
            this.Demos.Add(new DemoInfo() { SampleName = "Column Rearrangement", GroupName = "KANBAN BOARD", Tag = Tag.New, Description = "This example demonstrates column rearrangement in the Kanban control. You can drag the columns and drop them in any stage on the Kanban board.", DemoViewType = typeof(ColumnRearrangement) });
        }
    }
}
