...
namespace syncfusion.olapclientdemos.wpf
{
    ...
    /// </summary>
    public class VirtualKPIViewModel : NotificationObject, IDisposable
    {
        #region Members
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="VirtualKPIViewModel"/> class.
        /// </summary>
        public VirtualKPIViewModel()
        {
            if (AppDomain.CurrentDomain.BaseDirectory.Contains("Binaries_"))
            {
                ConnectionString = VirtualKPIModel.Initialize(System.IO.Path.GetFullPath(@"..\..\common\Assets\Config\OLAPSample.config"));
            }
            else
            {
                ConnectionString = VirtualKPIModel.Initialize(System.IO.Path.GetFullPath(@"..\..\..\common\Assets\Config\OLAPSample.config"));
