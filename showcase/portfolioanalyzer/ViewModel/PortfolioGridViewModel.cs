...
namespace syncfusion.portfolioanalyzerdemo.wpf
{
    ...
    public class PortfolioGridViewModel : INotifyPropertyChanged
    {
        ...
        private List<StocksGrid> stockList;
        ...
        {
            IsBusy = false;
            this.StockList = (e.Result as ObservableCollection<StocksGrid>).ToList();
        }

        void worker_DoWork(object sender, DoWorkEventArgs e)
        {
            IsBusy = true;
            e.Result = new ObservableCollection<StocksGrid>(GetHoldings(e.Argument.ToString()));
        }
        ...
        public List<StocksGrid> StockList
        {
            get
            {
                return stockList;
    ...
    public class DayChangeSummary : ISummaryAggregate
    {
         ...
         Action<System.Collections.IEnumerable, string, PropertyDescriptor>ISummaryAggregate.CalculateAggregateFunc()
        {
            return (items, property, pd) =>
            {
                ...
                var enumerableItems = items as IEnumerable<StocksGrid>;
                decimal? count = enumerableItems.Count();

                if (pd.Name == "DayChangeTotal")
                {
                    decimal? dayChange = 0;
                    foreach (StocksGrid stock in enumerableItems)
                    {
