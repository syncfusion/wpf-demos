    ...
    public class EmployeeInfoViewModel : NotificationObject, IDisposable
    {
        ...
        public void PopulateWithFlatData(int count)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                emp.EmpId = 1000 + i;
                emp.Rating = random.Next(1, 6);
                emp.Hike = hike[random.Next(hike.GetLength(0))];
                emp.CustomerCity = city[random.Next(city.GetLength(0))];
                emp.CustomerArea = "Area " + random.Next(1, 30);
                emp.OrderID = random.Next(1000, 9999);
                emp.UnitPrice = 100 + random.NextDouble() * 10000;
                emp.Discount = Math.Round(random.NextDouble() * 30, 2);
                emp.CustomerID = emp.LastName + random.Next(1, 100).ToString();
                emp.OrderDate = GenerateRandomDate().Date;
                emp.MyEyeColor = color[random.Next(color.GetLength(0))];
                emp.DOB = GenerateRandomDate();
                emp.DOJ = (random.Next(0, 5) == 0) ? (DateTime?)null : GenerateRandomDate();
                emp.JoiningDate = (random.Next(0, 6) == 0) ? (DateTime?)null : GenerateRandomDate();
                emp.ContactNumber = 9000000000 + i;
                emp.Availability = (random.Next(0, 2) == 0);
                emp.Date = emp.OrderDate.ToShortDateString();
                emp.ReportsTo = -1;
